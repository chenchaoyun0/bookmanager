...
//      TLog findOne = baseMongoRepository.findOne(query, TLog.class);
  ...
  public PagedResult<TLog> selectLogPagesByMongo(TLog tLog, Integer pageNo, Integer pageSize) {
    ...
    TypedAggregation<TLog> aggregationCount
      = Aggregation.newAggregation(TLog.class, Aggregation.group("userIp").sum("count").as("count"));
    ...
    TypedAggregation<TLog> aggregation
      = Aggregation.newAggregation(TLog.class, Aggregation.sort(Sort.Direction.DESC, "operTime"),
        Aggregation.group("userIp").max("operTime").as("operTime").sum("count").as("count").first("logId").as("logId")
          .first("userIp").as("userIp").first("userName").as("userName").first("userNickName").as("userNickName")
          .first("userAddress").as("userAddress").first("userJwd").as("userJwd").first("module").as("module")
          .first("action").as("action").first("actionTime").as("actionTime").first("operTime").as("operTime")
  ...
  public Long selectLogSumCountMongo() {
    Aggregation aggregation = Aggregation.newAggregation(Aggregation.group("$sum").sum("count").as("totalcount"));
    AggregationResults<BasicDBObject> aggregateCount
      = mongoTemplate.aggregate(aggregation, "tLog", BasicDBObject.class);
  ...
  public PagedResult<TLog> selectLogPagesForIpByMongo(String userIp, Integer pageNo, Integer pageSize) {
    ...
    TypedAggregation<TLog> aggregationCount
      = Aggregation.newAggregation(TLog.class, Aggregation.match(Criteria.where("userIp").is(userIp)));
    AggregationResults<BasicDBObject> aggregateCount = mongoTemplate.aggregate(aggregationCount, BasicDBObject.class);

    int total = aggregateCount.getMappedResults().size();

    TypedAggregation<TLog> aggregation = Aggregation.newAggregation(TLog.class,
      Aggregation.group("logId").max("operTime").as("operTime").sum("count").as("count").first("logId").as("logId")
        .first("userIp").as("userIp").first("userName").as("userName").first("userNickName").as("userNickName")
        .first("userAddress").as("userAddress").first("userJwd").as("userJwd").first("module").as("module")
        .first("action").as("action").first("actionTime").as("actionTime").first("operTime").as("operTime"),
      Aggregation.match(Criteria.where("userIp").is(userIp)), Aggregation.sort(Sort.Direction.DESC, "operTime"),
      Aggregation.skip(Long.parseLong((pageNo > 1 ? (pageNo - 1) * pageSize : 0) + "")), Aggregation.limit(pageSize));
